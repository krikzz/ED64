...
u8 bi_usb_rd(void *dst, u32 len) {
    ...
    while (len) {
        ...
        baddr = 512 - blen; //address in fpga internal buffer. requested data length equal to 512-int buffer address


        bi_reg_wr(REG_USB_CFG, USB_CMD_RD | baddr); //usb read request. fpga will receive usb bytes until buffer address reach 512

        resp = bi_usb_busy(); //wait until the requested data amount will be transferred to the internal buffer
...
u8 bi_usb_wr(void *src, u32 len) {
    ...
    while (len) {
        ...
        if (blen > len)blen = len;
        baddr = 512 - blen; //address in fpga internal buffer. data length is equal to 512-int buffer address
        ...
        resp = bi_usb_busy(); //wait until requested data amount is transferred
        if (resp)break; //timeout
...
u16 bi_old_sd_mode;
//this function gives time for stable values to be set on the open bus
...
//swap bytes copied from the SD card. It only affects reads to the ROM area
